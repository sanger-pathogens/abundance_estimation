...
# Modified by Yan Shao: 1) accept and write fastq.gz; 2) default opt-in (not opt-out) to produces a FASTQC report before and after the procedures.  	  #
###########################################################################################################################################################
...
help_message () {
	...
	echo "	--run-pre-qc-report	do make FastQC report of input sequences"
	echo "	--run-post-qc-report	do make FastQC report of final sequences"
	echo "";}
...
bmtagger=true; trim=true; pre_qc_report=false; post_qc_report=false
HOST=hg38

# load in params
OPTS=`getopt -o ht:o:1:2:x: --long help,skip-trimming,skip-bmtagger,run-pre-qc-report,run-post-qc-report -- "$@"`
...
while true; do
	case "$1" in
		...
		--run-pre-qc-report) pre_qc_report=true; shift 1;;
		--run-post-qc-report) post_qc_report=true; shift 1;;
...
if [ "$trim" = true ]; then
	...
	trim_galore --dont_gzip --no_report_file --paired -o $out $reads_1 $reads_2
...
if [ "$bmtagger" = true ]; then
	...
	${SOFT}/skip_human_reads.py ${out}/${sample}.bmtagger.list $reads_1 > ${out}/${sample}_clean_1.fastq
	${SOFT}/skip_human_reads.py ${out}/${sample}.bmtagger.list $reads_2 > ${out}/${sample}_clean_2.fastq

	comm "Now sorting out found human reads and putting them into a new file... for science..."
	${SOFT}/select_human_reads.py ${out}/${sample}.bmtagger.list $reads_1 > ${out}/${sample}_host_reads_1.fastq
	${SOFT}/select_human_reads.py ${out}/${sample}.bmtagger.list $reads_2 > ${out}/${sample}_host_reads_2.fastq
	if [[ ! -s ${out}/${sample}_clean_1.fastq ]]; then error "Something went wrong with removing contaminant reads with bmtagger. Exiting."; fi

	rm -r ${out}/bmtagger_tmp
	rm ${out}/${sample}.bmtagger.list
	reads_1=${out}/${sample}_clean_1.fastq
	reads_2=${out}/${sample}_clean_2.fastq
	host_reads_1=${out}/${sample}_host_reads_1.fastq
	host_reads_2=${out}/${sample}_host_reads_2.fastq

	gzip $reads_1 $reads_2 $host_reads_1 $host_reads_2
	rm ${out}/trimmed_1.fastq ${out}/trimmed_2.fastq
fi

comm "Contamination-free and trimmed reads are stored in: ${out}/${sample}_clean_1.fastq.gz and ${out}/${sample}_clean_2.fastq.gz"
...
if [ "$post_qc_report" = true ]; then
	...
	fastqc -t $threads -o ${out}/post-QC_report -f fastq $reads_1".gz" and $reads_2".gz"
